...
public class DocumentUploadController {
    ...
    public ResponseEntity<Map<String, Object>> initChunkedUpload(@RequestBody ChunkUploadRequest request) {
        try {
            String uploadId = uploadSessionManager.createSession(
                    request.getFileName(), request.getFileSize(), request.getTotalChunks());

            logger.info("初始化分片上传 [uploadId={}, fileName={}, chunks={}]",
                    uploadId, request.getFileName(), request.getTotalChunks());

            return ResponseEntity.ok(Map.of(
                    "uploadId", uploadId,
                    "uploadedChunks", Set.of()
            ));
        } catch (Exception e) {
    ...
    public ResponseEntity<Map<String, Object>> uploadChunk(
        ...
        try {
            boolean complete = uploadSessionManager.saveChunk(uploadId, chunkIndex, chunk.getInputStream());
