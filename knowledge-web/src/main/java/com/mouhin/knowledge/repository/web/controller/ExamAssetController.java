 ...
 * <p>
 * 位于 {@code /api/exam/assets/{assetKey}}，属考生侧放行前缀（{@link com.mouhin.knowledge.repository.web.security.AdminTokenAuthFilter}），
 * 浏览器 {@code <img>} 可直接拉取而无需附带令牌。鉴权通过<b>不可枚举的随机 {@code assetKey}</b> 承载
 * （UUID 无连字符，32 位）：仅持有句柄者可访问，句柄无效 / 缺失即 404，避免枚举与目录穿越。
 * 命中后按 {@code assetKey} 稳定不变的特点返回长时强缓存。
 * </p>
 *
