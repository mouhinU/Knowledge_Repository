...
- 临时文件必须在 `finally` 中清理。
- 新增文件格式须在 `DocumentExtractionService` 中添加专用解析器（检查清单第 17 项）。
...
- 检索 / 开考等带副作用的用例，其权限过滤与状态门禁落在单一 `CmdExe` / `QryExe`，事务边界在 app 层（模式见 [architecture-decisions.md](architecture-decisions.md) §4）。
