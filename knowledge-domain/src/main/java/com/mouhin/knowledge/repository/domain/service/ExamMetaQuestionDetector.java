 ...
 * <p>
 * 语文、英语等偏理解运用的科目里，「某知识在第几单元 / 第几课 / 第几页 / 哪一章 / 出自哪篇」这类
 * 题只考教材编排位置、不考内容本身，属低质量命题。本检测器以规则命中这类题干，用于两处确定性后处理：
 * </p>
 * <ol>
 *     <li>内容审核阶段（{@code exam-reviewer}）：命中即把分数压到阈值下、驱动 writer 打回重写；</li>
 *     <li>发布契约门禁（{@link ExamContractValidator}）：命中即产出 issue，强制人工校对、绝不自动发布。</li>
 * </ol>
 * <p>
 * 为压制误报，仅覆盖教材编排类名词（单元 / 课 / 页 / 册 / 章 / 节 / 章节 / 部分），刻意不含
 * 「段 / 篇」等常被用于指代试卷内短文段落的词；且必须与序数（第…）或疑问（哪…）搭配才判为位置题。
 * </p>
 *
...
public final class ExamMetaQuestionDetector {
    ...
    private static final Pattern[] META_PATTERNS = {
            ...
            // 出自 / 选自 / 来自 …… 哪 / 几 …… 单元|篇|课
            Pattern.compile("(?:出自|选自|来源于|来自|摘自|选取自)[^。？?]{0,8}(?:哪|几)[^。？?]{0,4}(?:单元|课|页|册|章节|章|节|篇)"),
            // 教材 / 课本 / 课文 / 书中 + 第X页|单元
     ...
     * <p>仅识别以题号起始的行（{@code **1.**} / {@code 1.} / {@code 1、}），据此取该题所在行做判定。</p>
     *
