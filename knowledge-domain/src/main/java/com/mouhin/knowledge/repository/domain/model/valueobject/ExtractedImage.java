 ...
 * 文档内嵌图片提取项（值对象）。
 * <p>
 * {@code DocumentImageExtractorGateway} 的返回契约：从原始文件中抽取到的单张位图，携带二进制与
 * 尺寸、来源页/序号等元信息，交由应用层落盘并持久化为 {@code DocumentImage} 实体。字节数组
 * （{@code byte[]}）是纯数据而非基础设施技术，故领域层可安全持有，符合依赖倒置。
 * </p>
 *
 * @param bytes     图片二进制内容
