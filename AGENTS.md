...
> 本文件用于指导 AI 生成代码与团队协作；其中“必须 / 禁止 / 应当”主要通过代码评审、门禁和规范执行落地，部分条目是约定而非 Java 编译器强制规则。
> **所有 AI 生成的代码必须严格遵循以下红线与对应分册。**

> 说明：本文使用 **COLA 规范术语**（如 adapter / app / client / domain / infrastructure），但当前仓库的**实际物理模块名**为 `knowledge-web` / `knowledge-application` / `knowledge-client` / `knowledge-domain` / `knowledge-infrastructure`。新增代码以物理模块和分册约定为准，命名与层职责以 [docs/architecture-decisions.md](docs/architecture-decisions.md) 为权威。
> 映射速记：`adapter ≈ knowledge-web`，`app ≈ knowledge-application`，`client ≈ knowledge-client`，`domain ≈ knowledge-domain`，`infrastructure ≈ knowledge-infrastructure`。
...
- 我要**改依赖 / 加库 / 动配置**？→ [docs/tech-stack.md](docs/tech-stack.md)（版本基线与版本纪律）。
