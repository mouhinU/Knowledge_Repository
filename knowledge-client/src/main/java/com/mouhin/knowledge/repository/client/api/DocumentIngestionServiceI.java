 ...
 *
 * <p>预览与「确认入库 / 重新入库」等用例出入参均为可序列化的 client DTO；策略以 {@code String}
 * 传入由 app 层解析，避免 client 依赖领域枚举。</p>
 *
 * <p>依赖 HTTP multipart 流（文件上传 / 直接解析）与 SSE 进度回调（异步索引）的用例与传输强耦合，
 * 不进对外契约，改由适配层直接调用 app 层执行器，保持 SSE 装配不变。</p>
 *
 * <p>校验失败以 {@link IllegalArgumentException} / {@link IllegalStateException} 抛出，由适配层
 * 或全局异常处理转换为响应。</p>
 *
...
public interface DocumentIngestionServiceI {
    ...
    /** 基于已上传文档解析预览（不入库） */
    PreviewResult previewFromDocument(String documentKey, int chunkSize, int overlap, String strategy);
