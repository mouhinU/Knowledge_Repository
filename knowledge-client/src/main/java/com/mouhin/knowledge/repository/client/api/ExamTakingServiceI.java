 ...
 * 适配层负责将其还原为前端所需的 JSON 形状。开考 / 交卷等校验失败仍以
 * {@link IllegalArgumentException} / {@link IllegalStateException} 抛出，由适配层转换为 400 / 409 响应。
 * {@link #validateReport} 的 data 为可直接序列化的 Map（等价于原 {@code PaperValidationReport#toMap}），
 * 避免 client 层引用 app 内部类型。</p>
 *
...
public interface ExamTakingServiceI {
    ...
    SingleResponse<ExamSessionDTO> startWithPaper(String studentToken, String examPaper, String answerKey, String topic, String difficulty);
