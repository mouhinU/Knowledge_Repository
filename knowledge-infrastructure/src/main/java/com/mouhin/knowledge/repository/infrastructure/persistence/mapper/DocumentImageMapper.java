...
public interface DocumentImageMapper extends BaseMapper<DocumentImageDO> {
    ...
    /**
     * 全局图片检索：联表来源文档带出文件名，支持按 documentKey 限定单篇文档， 或按 keyword 模糊匹配来源文档名 / 文档 Key；均为空则按配图 id 倒序返回全部。
     *
     * <p>SQL 兼容 H2 与 MySQL（LIMIT ? OFFSET ?、CONCAT）。{@code di.*} 经下划线转驼峰 映射到 {@link
     * DocumentImageSearchRowDO}（含联表别名 source_document_name）。
     */
    @Select(
            """
            <script>
            ...
            """)
    List<DocumentImageSearchRowDO> searchRows(
    ...
    long countSearchRows(
            @Param("keyword") String keyword, @Param("documentKey") String documentKey);
}
