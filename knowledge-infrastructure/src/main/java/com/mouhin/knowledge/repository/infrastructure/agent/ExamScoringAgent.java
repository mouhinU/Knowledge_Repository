 ...
 * 分值校验与评估 Agent
 *
 * <p>出卷流水线第 2 节点。<b>不再重排分值</b>——分值分配已由前置「题型分布方案」阶段生成并经用户确认； 本 Agent 只做：
 *
 * <ol>
 *   <li>硬校验：合计分值 = 本卷满分、每题 ≥ 1 分、perQuestion 长度 = count
 *   <li>合理性评估：单题占比、题型小计占比、题量均值、主客观题覆盖、满分是否标准（100/120/150）
 *   <li>不合格：抛出异常阻断流水线，向 SSE 上报"分值校验未通过 + 建议"，后续「试卷编写 / 答案生成 / 难度校准 / 内容审核 / 查重去重」节点均不再执行
 *   <li>合格：把方案渲染为 Markdown 塞入 blackboard.scoringScheme，供 Writer 严格遵循
 * </ol>
 *
...
public class ExamScoringAgent implements BlackboardAgent {
    ...
    public void execute(BlackboardState blackboard, BlackboardProgressCallback progressCallback) {
        ...
        String startMessage =
                skipValidation ? "正在写入分值方案（Node 2 已校验通过）..." : "正在校验题型分布方案的总分与分值分布...";
        emitProgress(
                progressCallback,
        ...
        if (!result.pass()) {
            ...
            emitProgress(
                    progressCallback, BlackboardProgressEvent.agentFailed("exam-scoring", report));
            String error =
                    "分值校验未通过，共 "
                            + result.issues().size()
    ...
    private void failPipeline(
            BlackboardProgressCallback callback, String report, String materials) {
    ...
    private String describeInput(ExamPlan plan, String questionConfig) {
        ...
        sb.append("主题输入源：");
        if (plan != null && plan.getTypes() != null && !plan.getTypes().isEmpty()) {
            sb.append("已确认的题型分布方案（")
                    .append(plan.getTypes().size())
                    .append(" 种题型，合计 ")
