 ...
 * LLM 对话模型配置
 *
 * <p>通过 knowledge.llm.provider 配置项切换对话模型提供商：
 *
 * <ul>
 *   <li>dashscope — 阿里云通义 DashScope（OpenAI 兼容端点）
 *   <li>deepseek — DeepSeek（OpenAI 兼容端点）
...
public class ChatModelConfig {
     ...
     *
     * <p>与上方三个 {@link ChatModel} Bean 使用完全一致的 provider 配置，只是改为 直接以 {@code stream=true} 请求 OpenAI
     * 兼容端点，从而能同时拿到正式输出与 思考链增量。dashscope / deepseek / ollama 共用同一实现，仅参数不同。
     */
    @Bean
    ...
    public StreamingChatGateway streamingChatGateway(
            ...
            @Value("${knowledge.llm.ollama.base-url:http://localhost:11434/v1}")
