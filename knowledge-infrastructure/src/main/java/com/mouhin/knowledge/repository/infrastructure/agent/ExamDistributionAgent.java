 ...
 * 题型分布方案 Agent（多阶段）
 * <p>
 * 依据"考试主题/科目 + 难度 + 学段"产出一份可编辑的题型分布方案，内部按四个阶段推进：
 * <ol>
 *     <li>题型分类：结合学科与学段选定合适的题型集合（内核题型固定 6 类，显示名可按科目命名）；</li>
 *     <li>题型数量：为每个题型确定题量（结合难度与目标题量）；</li>
 *     <li>每题分数：由 {@link ScoreRuleEngine} 确定性地把满分拆到每一道小题（Σ 恒等于满分）；</li>
 *     <li>合理性评估：LLM 复审方案，仅给出建议、不自动改写。</li>
 * </ol>
...
public class ExamDistributionAgent implements ExamDistributionGateway {
    ...
    public ExamPlan generate(String topic, String difficulty, String schoolLevelCode,
        ...
        logger.info("[Distribution] 开始生成方案 [topic='{}', difficulty='{}', level='{}', fullMark={}]",
                topic, difficulty, level, fullMark);
        ...
        for (TypePlan t : structure) {
            ...
            typeNames.append(t.getLabel()).append("（").append(ScoreRuleEngine.chineseFromKey(t.getKey())).append("）");
            countDetail.append("· ").append(t.getLabel()).append("：").append(t.getCount()).append(" 道");
        ...
        String think2 = "低学段/易难度侧重客观题、题量大而单题分值小；高学段/难难度提高主观题比重。"
                + "题量与满分匹配：总题量不宜超过满分（否则出现 0 分小题）。";
        ...
        String think3 = "以题型权重为系数，用最大余额法把满分分配到每一道小题，保证各题分值之和恰等于满分；"
                + "同题型内默认等值，用户可在页面上逐题调整。";
        ...
        for (TypePlan t : plan.getTypes()) {
            scoreDetail.append("· ").append(t.getLabel()).append("：每题 ").append(t.getPerQuestion())
                    .append(" 分，小计 ").append(t.subtotal()).append(" 分\n");
        }
        scoreDetail.append("合计：").append(plan.allocatedTotal()).append(" 分 / 满分 ").append(fullMark).append(" 分");
        ...
        logger.info("[Distribution] 方案生成完成，共 {} 题型 {} 题，满分 {} 分",
                plan.getTypes().size(), plan.totalQuestions(), plan.getTotalFullMark());
        return plan;
    }

    private void emitRunning(BlackboardProgressCallback cb, String agent,
                             String input, String thinking) {
        if (cb == null) {
            return;
        }
        cb.onProgress(new BlackboardProgressEvent.Builder()
                .type("AGENT_OUTPUT")
                .agentName(agent)
                .agentStatus("running")
                .message(thinking)
    ...
    private List<TypePlan> classifyAndCount(String topic, String difficulty,
        ...
        String systemPrompt = """
                ...
                """;
        String hint = (knowledgeHint != null && !knowledgeHint.isBlank())
    ...
    private List<String> evaluate(String topic, String difficulty, ScoreRuleEngine.SchoolLevel level, ExamPlan plan) {
        ...
        for (TypePlan t : plan.getTypes()) {
            schemeText.append(String.format("- %s（%s）：%d 道，小计 %d 分，逐题分值 %s\n",
                    t.getLabel(), t.getKey(), t.getCount(), t.subtotal(), t.getPerQuestion()));
        }
        String systemPrompt = """
    ...
    private static String text(JsonNode node, String field) {
        return node != null && node.has(field) && !node.get(field).isNull() ? node.get(field).asText("").trim() : null;
    }
