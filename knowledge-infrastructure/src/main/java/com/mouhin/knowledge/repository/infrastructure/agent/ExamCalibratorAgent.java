 ...
 * 难度校准 Agent
 * <p>
 * 分析试卷整体难度分布，与目标难度对比，给出调整建议。
 * 如果难度偏差较大，会输出调整后的试卷内容。
 * </p>
 * <p>
 * 读取：examPaper（试卷内容）、examDifficulty（目标难度）、question（主题）
 * 写入：difficultyAssessment（难度评估报告）
 * </p>
 *
...
public class ExamCalibratorAgent implements BlackboardAgent {
    ...
    public void execute(BlackboardState blackboard, BlackboardProgressCallback progressCallback) {
        ...
        String materials = examPaper != null && examPaper.length() > 400
                ? examPaper.substring(0, 400) + "..." : examPaper;
        ...
        if (examPaper == null || examPaper.isBlank()) {
            ...
            emitProgress(progressCallback, BlackboardProgressEvent.agentCompleted(
                    "exam-calibrator", "试卷为空，无法校准。"));
            return;
        }

        String difficultyDesc = switch (targetDifficulty) {
        ...
        emitProgress(progressCallback, BlackboardProgressEvent.agentCompleted("exam-calibrator", assessment));
