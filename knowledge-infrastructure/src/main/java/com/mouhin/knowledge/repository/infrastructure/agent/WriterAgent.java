...
public class WriterAgent implements BlackboardAgent {
    ...
    public void execute(BlackboardState blackboard, BlackboardProgressCallback progressCallback) {
        ...
        String materials = (findings != null && !findings.isBlank())
        ...
        if (findings == null || findings.isBlank()
                || findings.contains("未找到") || findings.contains("不足以回答")) {
            String fallback = "抱歉，知识库中没有足够的信息来生成文章。\n\n建议：\n1. 确认相关文档已上传并完成索引\n2. 尝试使用不同的关键词重新提问\n3. 检查知识库中是否包含与问题相关的内容";
            blackboard.setDraftArticle(fallback);
            emitProgress(progressCallback, BlackboardProgressEvent.agentCompleted("writer", fallback));
