 ...
 * 查重去重 Agent
 * <p>
 * 分析试卷内容的原创性，检查是否存在重复考查同一知识点或高度相似的题目。
 * </p>
 * <p>
 * 读取：examPaper（试卷内容）、answerKey（答案）、question（主题）
 * 写入：deduplicationReport（查重报告）
 * </p>
 *
    ...
    public void execute(BlackboardState blackboard, BlackboardProgressCallback progressCallback) {
        ...
        if (examPaper == null || examPaper.isBlank()) {
            ...
            emitProgress(progressCallback, BlackboardProgressEvent.agentCompleted(
                    "exam-deduplicator", "试卷为空，无法查重。"));
            return;
        }

        String answerContext = (answerKey != null && !answerKey.contains("失败"))
                ? "\n\n【标准答案】\n" + answerKey : "";
        ...
        emitProgress(progressCallback, BlackboardProgressEvent.agentCompleted("exam-deduplicator", report));
