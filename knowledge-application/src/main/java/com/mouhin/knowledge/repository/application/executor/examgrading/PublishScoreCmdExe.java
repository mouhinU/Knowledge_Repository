...
public class PublishScoreCmdExe {
    ...
    public void execute(Long sessionId, String reviewer) {
        ...
        long pendingReview = answers.stream().filter(PublishScoreCmdExe::requiresHumanConfirmation).count();
        if (pendingReview > 0) {
            throw new IllegalStateException("尚有 " + pendingReview
                    + " 道主观题 / 缺标准答案 / 未评分题项未经人工确认，无法发布成绩");
        }
     ...
     * 判断某题在发布前是否必须经人工确认。
     * <p>
     * 仅当"机器无法自信评分"时才拦门禁：主观题、缺少标准答案的客观题、或尚未评分的题项。
     * 客观题若标准答案存在，则自动比对结果（无论对错）都是确定的，无需人工复核即可发布，
