 ...
 *
 * <p>逻辑原样迁移自 {@code DocumentIngestionApplicationService.indexDocument}。当前线上走异步 SSE 入口，
 * 同步入口作为完整用例保留。
 *
 * <p>CONC-3 / OPS-2：本方法不再标注 {@code @Transactional}。实际工作委托给 {@link
 * DocumentIngestionSupport#processDocument} —— 该方法逐步自行落库并在异常时内部吞掉、 标记 FAILED
