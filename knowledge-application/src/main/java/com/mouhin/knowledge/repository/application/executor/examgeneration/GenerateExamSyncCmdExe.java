 ...
 * <p>因入参含领域类型 Permission / ExamPlan，不纳入 client 契约，由适配层直接调用。
 * 保留两条路径：按已确认方案，或按题型数量兜底。</p>
 *
...
public class GenerateExamSyncCmdExe {
    ...
    public String executeWithPlan(String topic, String difficulty, String schoolLevel, ExamPlan plan,
                                  Permission permission, String category) {
        return support.generateExam(topic, difficulty, schoolLevel, plan, permission, category);
    }

    public String executeByCounts(String topic, String difficulty, String schoolLevel,
                                  int singleChoice, int multiChoice, int trueFalse,
                                  int fillBlank, int shortAnswer, int essay,
                                  Permission permission, String category) {
        return support.generateExam(topic, difficulty, schoolLevel, singleChoice, multiChoice,
                trueFalse, fillBlank, shortAnswer, essay, permission, category);
    }
}
