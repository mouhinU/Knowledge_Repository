 ...
 *
 * <p>CONC-3 / OPS-2：事务边界仅覆盖关系库删除（chunk 行 + document 行，均为快 IO）， Milvus 向量删除移出事务。旧实现把 {@code
 * vectorStoreService.deleteByDocumentKey} 放在 {@code @Transactional} 方法的首行，虽在 DB 写之前调用，但事务在首次 SQL
