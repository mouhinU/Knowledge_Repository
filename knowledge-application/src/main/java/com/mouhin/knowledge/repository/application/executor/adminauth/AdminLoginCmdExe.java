 ...
 * 管理端登录命令执行器（app 层用例）。
 *
 * <p>校验用户名 + 密码（BCrypt）+ 账号激活态，通过后经 {@link AdminJwtService} 签发无状态 JWT 访问令牌。
 * 与考生登录不同：JWT 自包含身份与过期时间，<b>不落库 session_token</b>，故无需事务边界。</p>
 *
 * <p>安全考量：账号不存在与密码错误统一返回“用户名或密码错误”，避免用户名枚举；未设置密码
 * （passwordHash 为空）的账号同样按此提示拒绝。</p>
 *
...
public class AdminLoginCmdExe {
    ...
    private static final Logger logger = LoggerFactory.getLogger(AdminLoginCmdExe.class);

    private final UserGateway userGateway;
    private final BCryptPasswordEncoder passwordEncoder;
    private final AdminJwtService adminJwtService;

    public AdminLoginCmdExe(UserGateway userGateway,
    ...
    public AdminLoginResultDTO execute(AdminLoginCmd cmd) {
        ...
        User user = userGateway.findByUsername(cmd.getUsername())
                ...
                .filter(u -> passwordEncoder.matches(cmd.getPassword(), u.getPasswordHash()))
        ...
        String token = adminJwtService.issue(user.getUserKey(), user.getUsername(),
                Boolean.TRUE.equals(user.getAdmin()));
        AdminTokenPayload payload = adminJwtService.verify(token).orElseThrow(
                () -> new IllegalStateException("Issued token failed verification"));
        ...
        result.setExpiresAt(Optional.ofNullable(payload.expiresAt()).map(Object::toString).orElse(""));

        logger.info("管理端登录成功: username='{}', userKey={}", user.getUsername(), user.getUserKey());
        return result;
    }
}
