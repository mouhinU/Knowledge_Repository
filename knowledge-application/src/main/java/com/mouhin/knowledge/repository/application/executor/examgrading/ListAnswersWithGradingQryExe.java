 ...
 *
 * <p>答题记录的 {@code correctAnswer} 列在保存阶段并不落库（仅评分流程按需写入）， 因此对复核视图而言常为空。这里以「出卷即切分」的结构化题目行 （{@code
...
public class ListAnswersWithGradingQryExe {
    ...
    public ListAnswersWithGradingQryExe(
    ...
    /** 按权威题号定位结构化题目行：优先印刷题号（{@code question_number}）， 缺失时回退全局位置序号（与答案键题序一致）。 */
