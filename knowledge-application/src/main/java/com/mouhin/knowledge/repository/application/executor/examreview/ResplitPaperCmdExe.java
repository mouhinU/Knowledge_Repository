 ...
 * <p>
 * 按试卷原始 Markdown（exam_paper + answer_key + exam_plan）重新执行「出卷即切分」并幂等回灌
 * {@code kb_exam_question}，用于管理员修正原文或老数据首次结构化。重切分不改变试卷状态，
 * 发布仍须经 {@link ApprovePaperCmdExe} 通过契约校验。
 * </p>
 *
...
public class ResplitPaperCmdExe {
    ...
    public ResplitPaperCmdExe(PaperReviewSupport support,
    ...
    public ExamQuestionSplitSupport.SplitOutcome execute(String sessionKey) {
        ...
        logger.info("重新切分完成 [session={}, questions={}, pass={}]",
                sessionKey, outcome.count(), outcome.validation().pass());
        return outcome;
    }
}
