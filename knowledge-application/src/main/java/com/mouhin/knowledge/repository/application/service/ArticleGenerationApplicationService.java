 ...
 * <ol>
 *     <li>初始化黑板状态</li>
 *     <li>知识库检索（为研究员准备素材）</li>
 *     <li>研究员 Agent → 提取关键发现</li>
 *     <li>写手 Agent → 撰写文章草稿</li>
 *     <li>审核员 Agent → 审核并生成最终文章</li>
 * </ol>
 * 支持异步执行，通过回调接口实时推送进度事件。
 * </p>
 *
...
public class ArticleGenerationApplicationService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ArticleGenerationApplicationService.class);
    ...
    private final WritingHistoryRepository writingHistoryRepository;
    ...
    public ArticleGenerationApplicationService(
            ...
            WritingHistoryRepository writingHistoryRepository,
            ChatModel chatModel) {
        ...
        this.writingHistoryRepository = writingHistoryRepository;
        this.chatModel = chatModel;
    }

    /**
     * 异步启动文章生成，立即返回 sessionId。
     * <p>
     * 生成过程在后台线程中执行，通过 progressCallback 推送实时进度事件。
     * </p>
     *
     * @param question         用户问题
     * @param permission       用户权限上下文
     * @param progressCallback 进度回调（由控制器层提供，将事件推送到 SSE）
    ...
    private void saveHistory(String sessionId, String question, Permission permission,
        ...
        try {
            ...
            writingHistoryRepository.save(history);
        } catch (Exception ex) {
     ...
     * @return 历史记录列表
     */
    public List<WritingHistory> listHistory(int limit) {
        return writingHistoryRepository.listRecent(limit > 0 ? limit : 20);
    }

    /**
     * 根据 sessionId 查询写作历史详情
     *
     * @param sessionId 会话 ID
     * @return 历史记录，不存在时返回 null
     */
    public WritingHistory getHistoryBySessionId(String sessionId) {
        return writingHistoryRepository.findBySessionId(sessionId).orElse(null);
    }
     ...
     * 文章生成结果（保留用于同步调用场景）
     */
    public record ArticleGenerationResult(
            String sessionId,
