 ...
 * <p>
 * 教师对某份 AI 试卷点击「作废」时调用：
 * <ol>
 *   <li>把 {@code kb_exam_history.status} 由任意非 VOIDED 状态置为 {@link ExamHistory#STATUS_VOIDED}，
 *       并记录作废人与时间（复用 {@code reviewed_by / reviewed_time} 审计列）；</li>
 *   <li>级联把该试卷下所有考试场次 {@code kb_exam_session.voided} 置为 true，
 *       前端据此显示「试卷已作废」徽标；场次仍可显示与查阅，不影响答题记录与评分轨迹。</li>
 * </ol>
 * 若目标卷已是 VOIDED 则幂等直接返回。作废后学生不可再开考此卷，可用列表亦会自动移除（{@code listPublished}
 * 仅返回 PUBLISHED）。
 *
...
public class VoidPaperCmdExe {
    ...
    public VoidPaperCmdExe(ExamHistoryGateway examHistoryGateway,
    ...
    public int execute(String sessionId, String operator) {
        ...
        ExamHistory history = examHistoryGateway.findBySessionId(sessionId)
        ...
        logger.info("试卷已作废并级联场次 [session={}, operator={}, cascadedSessions={}]",
                sessionId, actor, cascaded);
        return cascaded;
    }
}
