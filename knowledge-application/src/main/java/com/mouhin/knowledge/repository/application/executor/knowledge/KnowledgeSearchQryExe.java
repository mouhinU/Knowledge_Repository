 ...
 *
 * <p>编排语义检索 + <b>权限过滤</b> + 应用层分类二次过滤 + 文档名补全，并组装为
 * {@link SearchResponseVO}。SEC-1 修复：权限过滤由 {@link AuthorizedSearchSupport} 在检索出口
 * 通过领域 {@code PermissionDomainService.hasAccess} 逐条落地，此前 {@code MilvusVectorStoreService
 * .buildFilter} 直接 {@code return null} 导致表达式被丢弃、跨权限文档泄露。默认参数、score 4 位
 * 小数取整与空值归一逻辑与原 {@code KnowledgeQueryApplicationService} + 控制器 Map 组装完全一致。</p>
 *
...
public class KnowledgeSearchQryExe {
    ...
    public KnowledgeSearchQryExe(AuthorizedSearchSupport authorizedSearch,
    ...
    public SearchResponseVO execute(SearchCmd cmd) {
        ...
        List<SearchResult> results =
                authorizedSearch.searchAuthorized(query, max, min, filterExpr, category, permission);
