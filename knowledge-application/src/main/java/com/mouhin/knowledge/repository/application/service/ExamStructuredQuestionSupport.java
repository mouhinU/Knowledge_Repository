 ...
 * <p>
 * 「出卷即切分」后 {@code kb_exam_question} 是标准答案 / 解析 / 评分标准的唯一权威来源，
 * 评分链路已纯读该结构。本支撑供下游展示用例（成绩复核详情、错题本）以同一口径纯读结构，
 * 消除对 {@code answer_key} 自由文本的重复解析（双口径残留）。场次与试卷的结构化题目以
 * 出卷会话 {@code session_id} 为键，故先解析出「试卷主键」再按印刷题号建映射。
 * </p>
 *
 * <p>本类只做读取，不触发切分回灌：这些展示用例仅作用于已评分场次，其结构行在评分期
 * （{@code ExamGradingSupport} 的惰性回灌）即已就绪。</p>
 *
...
public class ExamStructuredQuestionSupport {
    ...
    public ExamStructuredQuestionSupport(ExamHistoryGateway examHistoryGateway,
