 ...
 * 智能体 / 评分异步任务线程池工厂。
 * <p>
 * 统一收口应用层异步任务的线程资源，替代原先散落的
 * {@code Executors.newVirtualThreadPerTaskExecutor()}：后者无界创建任务、且从不关闭，
 * 一旦出卷扇出或批量评分瞬时涌入过多任务，会放大对 Ollama / 向量库 / 连接池的并发压力，
 * 容器优雅停机时也会遗留在途任务。按《Java 开发手册》与项目 AGENTS.md 第六章，
 * 线程池必须经 {@link ThreadPoolExecutor} 显式构造，此处给出合规且有界的默认构造。
 * </p>
 *
 * <p>线程数与队列显式命名，避免魔法值散落。</p>
 *
...
public final class AgentExecutorFactory {
    ...
    /**
     * 最大并发线程数上限：约束同时在途的 LLM / 出卷 / 评分任务数量，
     * 防止瞬时扇出压垮模型服务与数据库连接池。
     */
    ...
    private static final class NamedDaemonThreadFactory implements ThreadFactory {
        ...
        public Thread newThread(Runnable r) {
            ...
            t.setUncaughtExceptionHandler((thread, ex) ->
                    System.err.println("[AgentExecutor] 线程 " + thread.getName()
                            + " 未捕获异常: " + ex));
            return t;
        }
    }
