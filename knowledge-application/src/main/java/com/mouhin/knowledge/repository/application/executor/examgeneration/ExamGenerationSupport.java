...
public class ExamGenerationSupport {
    ...
    /** 后台执行完整的试卷生成流水线 */
    private void executeExamPipeline(
            String sessionId,
            String topic,
            String difficulty,
            String questionConfig,
        ...
        try {
            ...
            CompletableFuture<Void> scoringFuture =
                    CompletableFuture.runAsync(
            ...
            for (int attempt = 0; attempt <= MAX_REVIEW_RETRIES; attempt++) {
                ...
                if (attempt >= 1 && callback != null) {
                    ...
                    callback.onProgress(
                            BlackboardProgressEvent.retryRoundChanged(
                                    BlackboardPhase.WRITING,
                                    String.format(
                                            "上一轮未达标，正在重新生成试卷（第 %d / 共 %d 轮）...",
                ...
                CompletableFuture<Void> answerFuture =
                        CompletableFuture.runAsync(
                                ...
                                agentExecutor);
                CompletableFuture<Void> calibratorFuture =
                        CompletableFuture.runAsync(
    ...
    public String generateExam(
        ...
        String schemeText =
                ScoreRuleEngine.renderScheme(ScoreRuleEngine.allocate(total, counts), topic, level);
    ...
    public ExamPlan generateDistribution(
            String topic,
            String difficulty,
            String schoolLevel,
            String category,
        ...
        CompletableFuture.runAsync(
                () -> {
                    try {
                        ...
                        try {
                            String filterExpr =
                                    permissionDomainService.buildFilterExpression(permission);
                            int maxResults =
                                    searchMaxResults > 0 ? searchMaxResults : DEFAULT_MAX_RESULTS;
                            double minScore =
                                    searchMinScore > 0 ? searchMinScore : DEFAULT_MIN_SCORE;
                            List<SearchResult> results =
                                    authorizedSearch.searchAuthorized(
                                            topic,
                                            maxResults,
                        ...
                        com.mouhin.knowledge.repository.domain.service.ScorePlanValidator.Result
                                result =
                                        com.mouhin.knowledge.repository.domain.service
                                                .ScorePlanValidator.validate(plan);
                        String report =
                                com.mouhin.knowledge.repository.domain.service.ScorePlanValidator
     ...
     *
     * <p>仅为增强可读性，<b>不</b>参与门禁判定；模型异常时返回 {@code null}， 校验结论仍由 {@link
     * com.mouhin.knowledge.repository.domain.service.ScorePlanValidator} 决定。
     */
    private String streamValidationAnalysis(
            ExamPlan plan, String report, BlackboardProgressCallback progressCallback) {
        if (progressCallback == null) {
            return null;
        }
        try {
            ...
            String userPrompt =
                    ...
                    """,
                            ...
                            planText,
                            trimText(report, 800));
            return streamingChatGateway.streamCompletion(
                    systemPrompt,
                    userPrompt,
                    (kind, delta) ->
                            progressCallback.onProgress(
    ...
    public String generateExam(
        ...
        String userPrompt =
                buildUserPromptFromPlan(
                        topic, difficulty, plan, knowledgeContext, total, schemeText);
        ChatRequest request =
                ChatRequest.builder()
                        .messages(
    ...
    private String buildUserPromptFromPlan(
            String topic,
            String difficulty,
            ExamPlan plan,
     ...
     *
     * <p>契约校验不通过（含切分异常）→ {@code VALIDATION_FAILED}，强制人工校对，绝不自动发布； 校验通过且 {@code
     * review-required=false} 且质量分 ≥ 阈值 → {@code PUBLISHED}（自动发布）； 其余通过情形 → {@code REVIEWABLE}（等待管理员
     * / 出题人校对批准后发布）。
     *
