 ...
 * 归档文档命令执行器（app 层用例）
 * <p>
 * CONC-3 / OPS-2：归档状态落库置于短事务内（单次快 IO 更新），Milvus 向量删除移出事务。
 * 旧实现中 {@code vectorStoreService.deleteByDocumentKey} 处于 {@code @Transactional} 方法体内，
 * DB 连接自首次 SQL 起一直被占用到方法返回，Milvus 抖动时会拖长事务、耗尽连接池。
 * 现"DB 先、慢 IO 后"：状态更新成功提交即视为归档完成；向量删除失败仅记录，
 * 检索侧以 DB 状态为准，已归档文档不会再被召回（残留向量可由对账回收）。
 * </p>
 *
...
public class DocumentArchiveCmdExe {
    ...
    public void execute(String documentKey) {
        ...
        try {
            ...
            logger.warn("归档文档向量删除失败（状态已置归档，向量残留待对账回收）[documentKey={}]: {}",
