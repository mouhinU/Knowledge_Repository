 ...
 * <p>刻意<b>不加</b> {@code @Transactional}：评分含多次（主观题）LLM 流式调用，耗时可达分钟级，
 * 若包在事务内将长时间占用数据库连接并对场次行持有 X 锁，定时任务批量评分时极易耗尽连接池、
 * 且阻塞超时回收 CAS。并发正确性改由评分入口的原子状态机保证——
 * {@code SUBMITTED→GRADING} CAS 认领 + 每题心跳续约 + 终态 {@code GRADING→AI_GRADED} CAS 落库，
 * 与异步评分链路（本就无事务）保持一致。</p>
 *
