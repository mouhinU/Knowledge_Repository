 ...
 * 学生端「可用考试」入口专用：
 * <ol>
 *   <li>仅返回 {@code PUBLISHED} 状态试卷（未发布 / 待校对 / 校验不通过 / 已作废的卷不出现）；</li>
 *   <li>传入 {@code studentToken} 时，进一步剔除该考生已开考过的试卷（一人一卷一次系统级限制）；
 *       未登录或不传 token 时保持原全量返回语义，以兼容旧行为。</li>
 * </ol>
 *
...
public class ListPublishedHistoryQryExe {
    ...
    public List<ExamHistoryDTO> execute(int limit, String studentToken) {
        ...
        Set<Long> takenHistoryIds = examSessionGateway.listExamHistoryIdsByStudentId(student.getId())
                .stream()
                .collect(Collectors.toSet());
        // 已考卷剔除后可能不足 limit，向 DB 多取若干条后再截断
        List<ExamHistoryDTO> filtered = examHistoryGateway.listPublished(safeLimit * OVERFETCH_FACTOR).stream()
