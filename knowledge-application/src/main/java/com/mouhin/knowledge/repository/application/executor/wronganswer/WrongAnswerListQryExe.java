...
public class WrongAnswerListQryExe {
    ...
    private static final List<String> GRADED_STATUSES =
            List.of("AI_GRADED", "REVIEWED", "PUBLISHED");
    ...
    public List<WrongAnswerVO> execute(Long studentId, String topic, String questionType) {
        ...
        // 3. 筛选错题（得分 < 满分）
        List<ExamAnswer> wrongAnswers =
                allAnswers.stream().filter(a -> a.getEffectiveScore() < a.getMaxScore()).toList();
        ...
        // 5. 构建场次映射
        Map<Long, ExamSession> sessionMap =
                sessions.stream().collect(Collectors.toMap(ExamSession::getId, s -> s));
        ...
        for (ExamSession session : sessions) {
            questionBySession.put(
                    session.getId(), structuredQuestionSupport.loadByQuestionNumber(session));
        }
